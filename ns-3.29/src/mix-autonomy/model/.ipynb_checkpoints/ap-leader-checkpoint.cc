...
void APLeader::ReceivePacket(Ptr<Packet> pkt, Address &srcAddr)
{
...
void APLeader::ReceivePacketFromAP(Ptr<Packet> pkt)
{
...
void APLeader::SetupHeader(PacketHeader &hdr)
{
...
void APLeader::SlotAllocation()
{
  m_SCHslotAllocation.clear();
  if (m_CCHslotAllocation.size() == 0)
  {
    for (uint32_t i = 0; i < curSlot.apCCHSlotNum; i++)
      m_SCHslotAllocation.push_back(config.apNum - i);
    m_SCHslotAllocation[0] = config.apNum + 1;
